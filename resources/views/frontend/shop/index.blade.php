                            ...
                            <div class="product-shoe-info shoe">
                                <div class="men-pro-item">
                                    <div class="men-thumb-item">
                                        ...
                                        <div class="men-cart-pro">
                                            <div class="inner-men-cart-pro">
                                                <a href="{{ route('shop.show',encrypt($item->id)) }}" class="link-product-add-cart">Quick View</a>
                                            </div>
                                        </div>
                                    ...
                                    <div class="item-info-product">
                                        <h4>
                                            <a href="{{ route('shop.show',encrypt($item->id)) }}">{{ $item->name }}</a>
                                        </h4>
                                        <div class="info-product-price">
                                            <div class="grid_meta">
                                                <div class="product_price">
                                                    <div class="grid-price ">
                                                        <span class="money ">Total Suppliers : {{ count(\App\supplierModel::where('supplier_type',$item->id)->get()) }}</span>
                                                        <br>
                                                        @php $count = 0 @endphp
                                                        @foreach( \App\productTypeModel::where('supplier_type',$item->id)->get() as $productType)
                                                            @php
                                                                $data = \App\productModel::where('product_type', $productType->id)->count();
                                                                if($data){
                                                                    $count += $data;
                                                                }
                                                            @endphp
                                                        @endforeach
                                                        <span class="money ">Total Products &nbsp;: {{ $count }}</span>
                                                    </div>
                                                </div>
                                            </div>
